<script lang="ts">
    ...
    import { Sun, Moon, Github, Import } from "lucide-svelte";
    ...
    export let onConfigClick: (route: string) => void;
    ...
    $: mainRoutes = routes.filter((route) => route.type === "route");
    $: settingsRoute = routes.find((route) => route.type === "settings");
    $: customRoutes = routes.filter((route) => route.type === "custom");
...
<div class="flex flex-col h-full">
    ...
    <nav class="grid gap-2 mt-auto pb-4 justify-center">
        ...
        {#each customRoutes as route}
            <Tooltip.Provider>
                <Tooltip.Root delayDuration={0}>
                    <Tooltip.Trigger>
                        <Button size="icon" variant="secondary" onclick={() => onConfigClick(route.label)}>
                            <route.icon />
                        </Button>
                    </Tooltip.Trigger>
                    <Tooltip.Content side="right" class="flex items-center gap-4">
